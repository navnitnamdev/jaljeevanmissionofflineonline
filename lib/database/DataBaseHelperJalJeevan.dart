...
class DatabaseHelperJalJeevan {
  ...
  DatabaseHelperJalJeevan.internal();
  static const String localmasterdatatable = 'localmasterdatatable';
  ...
  _oncreate(Database db, int version) async {
    ...
    await db.execute(
        "CREATE TABLE Local_PWSSavedatato_server(id INTEGER PRIMARY KEY AUTOINCREMENT, "
            "Userid TEXT, Villageid TEXT, Assettaggingid TEXT, Stateid TEXT,"
            " Schemeid TEXT,schemename TEXT , "
            "blockName TEXT ,villageName TEXT,sourceName TEXT,sourceType TEXT,panchayatName TEXT, "
            "sourceTypeCategoryId TEXT, DivisionId TEXT, "
            "habitationName TEXT,habitationId TEXT,  landmark TEXT, latitude TEXT, "
            "longitude TEXT, Accuracy TEXT, Image TEXT, SourceId TEXT,subsourceaddnew TEXT ,"
...
/*  Future<void> alterTable() async {
    ...
    await dbClient?.execute(
      "ALTER TABLE localmasterschemelist ADD COLUMN source_type TEXT",
    );
  ...
  /* Future<void> _onUpgrade(Database db, int oldVersion, int newVersion) async {
    if (oldVersion < newVersion) {
      ...
      // Check if the 'source_type' column exists in 'localmasterschemelist'
      ...
      print("Has source_type column: $hasSourceTypeColumn");
  ...
  Future<void> _onUpgrade(Database db, int oldVersion, int newVersion) async {
    if (oldVersion < newVersion) {
      ...
      bool hassourceTypecategory = columns.any((column) => column['name'] == 'source_typeCategory');
  ...
  Future<Localmasterdatamodal_VillageDetails> insertMastervillagedetails(
      ...
      localmasterdatamodalVillagedetails) async {
    var dbClient = await db;

    await dbClient!.insert('localmastervilagedetailstable',
        localmasterdatamodalVillagedetails.toMap());
    return localmasterdatamodalVillagedetails;
  }

  Future<Localmasterdatamoda_Scheme> insertMasterSchmelist(
      Localmasterdatamoda_Scheme localmasterdatamodaScheme) async {
    var dbClient = await db;
    await dbClient!

        .insert('localmasterschemelist', localmasterdatamodaScheme.toMap());
    return localmasterdatamodaScheme;
  }
...
*/
  ...
  Future<LocalStoragestructureofflinesavemodal> insertstoragestructureofflinesaveindb(
  ...
  Future<void> removeEntries(String villageId, String schemeId) async {
    var dbClient = await instance.db;
    await dbClient!.delete(
      'Local_PWSSavedatato_server',
      where: 'Villageid = ? AND schemeId = ?',
  ...
  Future<void> removeEntriessibwhere(String villageId, String schemeId) async {
    var dbClient = await instance.db;
    await dbClient!.delete(
      'sibsavedatatable',
      where: 'villageId = ? AND schemeId = ?',
  ...
  Future<void> removeEntries_ss_where(String villageId, String schemeId) async {
    var dbClient = await instance.db;
    await dbClient!.delete(
    ...
    print('Entries removed successfully!');
  }  Future<void> removeEntries_ot_where(String villageId, String schemeId) async {
    var dbClient = await instance.db;
    await dbClient!.delete(
  ...
  Future<void> duplicate_entryslectedvillages() async {
    var dbClient = await instance.db;
    String query = '''
    DELETE FROM localmasterdatatable
    WHERE id NOT IN (
