...
class _AddNewSourceScreenState extends State<AddNewSourceScreen> {
  ...
  Future<void> _loadCustomFont() async {
    final fontData = await rootBundle.load('assets/Roboto-Black.ttf.zip');
    //final fontImageData = await rootBundle.load('assets/fonts/Roboto-Black.png');
    final font = img.BitmapFont.fromZip(fontData.buffer.asUint8List());
    print("font"+ fontData.toString());
    setState(() {
      _customFont = font;
    });
  }
  ...
  /*void openCamera() async {
    ...
    final targetPath = '${dir.absolute.path}/${DateTime
        .now()
        .millisecondsSinceEpoch}.jpg';

    print(targetPath);
    print(imgCamera.path);
    ...
    print("abc" + result!.path);

    final watermarkedImage = Drawlatlong.drawlatlong(
        result!.path,
        '37.7749', // Replace with actual latitude
        '-122.4194',
        _customFont // Replace with actual longitude
    );
    print("def");
    print(watermarkedImage);
    final directory = await getApplicationDocumentsDirectory();
    final path = '${directory.path}/watermarked_image.jpg';
    File(path).writeAsBytesSync(img.encodeJpg(watermarkedImage));
    print("abc");
  ...
  void openCamera() async {
    try {
     ...
     // getLocationDataFromImage(result!.path);

      if (result != null) {
        print("resultfor" +result.toString());
        ...
        if (kDebugMode) {
          ...
          print('Compressed image size kb : ${newKb} kb');
        }
  ...
  Future<GeoPoint?> getLocationDataFromImage(String filePath) async {
    ...
    final latLong = GeoPoint(latitude: 12.333, longitude: 32.332);
    print("gggggg" +latLong.toString());
    await exif.close();
   /* if (latLong != null) {
                                    ...
                                    imgFile == null
                                      ...
                                      child: Container(
                                        ...
                                        child:/*  _isLoading==true
                                            ? const Center(child: SizedBox(width:15 , height:15 ,child: CircularProgressIndicator(strokeWidth: 0.5 ,))) // Show CircularProgressIndicator when loading
                                            :*/Image.file(
                                          imgFile!,
                                          width: 260,
                                          height: 200,
                                  ...
                                  if (widget.Sourceid_typesend == "6") {
                                    ...
                                    /* if (selectradiobutton.toString() == "") {
                                      ...
                                      if (exists != null && exists) {
                                        ...
                                        databaseHelperJalJeevan?.insertpwssourcelocal(
                                            LocalPWSSavedData(
                                              userId: box.read("userid"),
                                              villageId:
                                              widget.villageid.toString(),
